...
- Code, projects, files, and folders use the `Journal` prefix (for example `Journal.Server`, `Journal.UI`).
...
- Frontend code must build request URLs from `API_BASE_URL` in `Journal.UI/src/utils/constants.ts`. Relative `/api/...` fetches break because the UI and API are separate origins.
